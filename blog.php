...
programs for aspiring business leaders.',
  ...
  [
   ...
   [
    'id' => 17,
    'title' => 'BCA Vs BTech: Which is Better After 12th?',
    'url' => 'bca-vs-btech-comparison',
    'date' => '2026-03-06',
    'excerpt' => 'Complete comparison between BCA and BTech covering eligibility, fees, duration, and career scope for students planning IT careers.',
    'author' => ['name' => 'GNC Dehradun'],
    'tags' => ['BCA', 'BTech'],
    ...
    if ($featured):
      ...
      <article class="hero-card" aria-labelledby="hero-title">
        ...
        <div class="hero-body">
          ...
          <p class="meta">By <?= htmlspecialchars($featured['author']['name']) ?> • <?= htmlspecialchars($featured['date']) ?>
          </p>
    ...
    <section class="posts" aria-live="polite">
      <div class="posts-grid" id="posts-grid">
        <?php foreach ($posts as $post): ?>
          <article class="post-card-blog" data-id="<?= intval($post['id']) ?>"
            data-title="<?= htmlspecialchars(strtolower($post['title'])) ?>"
            data-excerpt="<?= htmlspecialchars(strtolower($post['excerpt'])) ?>"
            ...
            <?php endif; ?>
            <div class="post-body">
              <h3 class="post-title"><a href="<?= urlencode($post['url']) ?>.php"><?= htmlspecialchars($post['title']) ?></a>
              </h3>
...
<style>
  ...
  * {
  ...
  .pageBanner-inner .pageBanner-inner_in {
    padding: 56px 0;
  }
  ...
  .post-card-blog:hover .thumb img {
  ...
  @media (max-width:1100px) {
    ...
    /* collapse to single column to give more room */
    .filters {
      position: relative;
      top: 0;
      margin-bottom: 1rem;
  ...
  .post-card-blog .thumb img[src=""],
  .post-card-blog .thumb img[alt=""] {
    object-fit: contain;
    background: #f5f5f7;
  }
